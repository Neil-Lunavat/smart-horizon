...
| **120 physics-generated windows**        | Beats the 7-day average in **all 60 GEO windows** and **78%** of the 60 MEO windows. Each window is a different satellite, with its own orbital period, phase, drift and noise draw, and a day-8 truth the model is never shown. Winning on nearly every one of them matters more than the size of any single win: it cannot be carried by a few lucky draws           |
...
"Doing nothing" is the trivial baseline: predicting that the error stays at zero for
the whole of day 8.
