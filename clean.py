#!/usr/bin/env python3
"""Loader and cleaner for the SH-DST-03 files, plus the three train/test pairings.
...
# column spellings we accept from an arbitrary supplier and map onto COLS
RENAME = {"clock_error": "satclockerror", "clock": "satclockerror",
          "satclock": "satclockerror", "x_err": "x_error", "y_err": "y_error",
          "z_err": "z_error", "time": "utc_time", "timestamp": "utc_time",
          "epoch": "utc_time"}


def normalise(df):
    """Headers and timestamps only. No rows are added, removed or reordered.

    `x_error (m)` -> `x_error`; alternative spellings mapped onto COLS; the US
    m/d/Y stamp parsed explicitly rather than left to the dateutil guesser, which
    reads 9/1 as 1 Sep on some rows and 9 Jan on others once the day exceeds 12.
    """
    df = df.copy()
    ...
    df["utc_time"] = t
    return df.dropna(subset=["utc_time"])[["utc_time"] + COLS]


def dedupe(df):
    """Drop exact repeats and sort. Returns (df, n_rows_removed).

    Lossless for these files: each is one block written twice (see the module
    docstring), so every dropped row is byte-identical to one that remains.
    """
    n0 = len(df)
    out = df.drop_duplicates().sort_values("utc_time").reset_index(drop=True)
    return out, n0 - len(out)


def read_any(path):
    """Read ANY 7-day error CSV from disk and clean it. Returns (df, n_removed).

    This is the entry point for data we were not given -- an evaluation file, a
    different constellation. `load` below is the same thing pinned to `data/`.
    """
    df, dropped = dedupe(normalise(pd.read_csv(path)))
    return df, dropped


def load(name, clean=True):
    """Read one delivered CSV from `data/`. `clean=False` returns it as delivered."""
    df = normalise(pd.read_csv(os.path.join(DATA, name)))
    return dedupe(df)[0] if clean else df
