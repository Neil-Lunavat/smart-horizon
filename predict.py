...
from clean import COLS, read_any                     # noqa: E402


def read_query_times(path):
    """Read a one-column CSV of timestamps to predict at."""
    q = pd.read_csv(path)
    col = ([c for c in q.columns if "time" in str(c).lower()] or [q.columns[0]])[0]
    t = pd.to_datetime(q[col], format="%m/%d/%Y %H:%M", errors="coerce")
    if t.isna().any():
        t = pd.to_datetime(q[col], errors="coerce")
    return t.dropna()
...
def main(argv=None):
    ...
    try:
        df, dropped = read_any(a.input)             # clean.py does the cleaning
    except ValueError as e:
        raise SystemExit(f"ERROR: {a.input}: {e}")
    if len(df) < 20:
